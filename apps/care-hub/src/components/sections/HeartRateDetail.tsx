  ...
  return (
    <>
          ...
          <div className="mt-3 flex flex-col gap-3">
            {zones.map((zone) => (
              <div key={zone.label} className={`flex items-center justify-between rounded-xl px-3 py-3 ${zone.bar}`}>
                ...
                <span className={`h-3 w-3 rounded-full ${zone.color} ${zone.status ? "motion-pulse" : ""}`} />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-4 rounded-2xl bg-white p-4 shadow-md motion-fade-up delay-3 card-hover">
          ...
          <div className="mt-2 h-4 w-full rounded-full overflow-hidden bg-gray-200 flex motion-shimmer">
